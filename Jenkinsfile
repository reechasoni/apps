node('master'){
	...
	stage('Delete Old Container'){
	   sshagent (credentials: ['dev-docker']) {
	       try{
	         ...
	         sh "ssh -o StrictHostKeyChecking=no ec2-user@172.31.17.236  ${dockrRm} "
			 sh "ssh -o StrictHostKeyChecking=no ec2-user@172.31.17.236  ${dockrRmImage} "
		   }catch(e){
	...
	stage('Deploy On Dev'){
		sshagent (credentials: ['dev-docker']) {
		 ...
		 sh "ssh -o StrictHostKeyChecking=no ec2-user@172.31.17.236  ${dockerRun} "
		}
	}
